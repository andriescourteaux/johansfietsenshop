...
Assert-Matches $winterMainSection '(?is)<section class="[^"]*\bpage-intro--winter\b' 'winter maintenance page styling hook'
...
Assert-Matches $cssContent '(?is)\.page-intro--winter\s+h1\b[^{}]*\{(?=[^}]*max-width\s*:\s*52rem)(?=[^}]*font-size\s*:\s*clamp\(2rem,\s*4vw,\s*3\.25rem\))(?=[^}]*line-height\s*:\s*1\.05)' 'assets/css/style.css polished winter page title'
Assert-Matches $cssContent '(?is)\.winter-page\b[^{}]*\{(?=[^}]*gap\s*:\s*clamp\(1\.5rem,\s*3vw,\s*2\.5rem\))(?=[^}]*margin-top\s*:\s*clamp\(1\.25rem,\s*3vw,\s*2rem\))' 'assets/css/style.css compact winter page spacing'
Assert-Matches $cssContent '(?is)\.winter-page\b[^{}]*\{(?=[^}]*padding\s*:\s*clamp\(1\.25rem,\s*3vw,\s*2rem\))(?=[^}]*background\s*:\s*#f4f4f4)(?=[^}]*box-shadow\s*:\s*0\s+0\s+0\s+100vmax\s+#f4f4f4)(?=[^}]*clip-path\s*:\s*inset\(0\s+-100vmax\))' 'assets/css/style.css winter page grey support plane'
Assert-Matches $cssContent '(?is)\.winter-page__copy\b[^{}]*\{(?=[^}]*padding\s*:\s*clamp\(1rem,\s*2\.5vw,\s*1\.65rem\))(?=[^}]*background\s*:\s*#fff)(?=[^}]*box-shadow\s*:\s*0\s+16px\s+40px\s+rgba\(23,\s*18,\s*47,\s*0\.08\))' 'assets/css/style.css winter copy white support plane'
Assert-Matches $cssContent '(?is)\.winter-page__copy\s+h2\s*,\s*\.winter-values\s+h2\b[^{}]*\{[^}]*margin\s*:\s*0\s+0\s+0\.5rem' 'assets/css/style.css compact winter heading spacing'
Assert-Matches $cssContent '(?is)\.winter-page__copy\s+p\b[^{}]*\{[^}]*line-height\s*:\s*1\.55' 'assets/css/style.css compact winter copy line height'
Assert-Matches $cssContent '(?is)\.winter-values\b[^{}]*\{[^}]*display\s*:\s*grid[^}]*gap\s*:\s*0\.75rem' 'assets/css/style.css compact winter values sections'
...
Assert-Matches $cssContent '(?is)\.winter-values__grid\s*>\s*\.page-value\b[^{}]*\{(?=[^}]*padding\s*:\s*clamp\(1rem,\s*2vw,\s*1\.35rem\))(?=[^}]*background\s*:\s*#fff)(?=[^}]*text-align\s*:\s*left)(?=[^}]*box-shadow\s*:\s*inset\s+0\.3rem\s+0\s+0\s+var\(--accent\),\s*0\s+14px\s+32px\s+rgba\(23,\s*18,\s*47,\s*0\.07\))' 'assets/css/style.css winter value white support cards'
