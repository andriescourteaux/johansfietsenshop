...
Assert-Matches $cssContent '(?is)\.home-overview__grid\s*\+\s*\.home-mode-sections::before\b(?=[^{}]*\{[^}]*display\s*:\s*block)(?=[^{}]*\{[^}]*height\s*:\s*clamp\(7\.5rem,\s*15vw,\s*12rem\))(?=[^{}]*\{[^}]*margin-bottom\s*:\s*clamp\(9rem,\s*17vw,\s*14rem\))(?=[^{}]*\{[^}]*background\s*:\s*#f4f4f4)(?=[^{}]*\{[^}]*box-shadow\s*:\s*0\s+0\s+0\s+100vmax\s+#f4f4f4)' 'assets/css/style.css homepage separator plane'
