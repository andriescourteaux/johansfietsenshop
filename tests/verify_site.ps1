...
Assert-Matches $cssContent '(?is)\.home-overview__grid\s*\+\s*\.home-mode-sections\b(?=[^{}]*\{[^}]*position\s*:\s*relative)(?=[^{}]*\{[^}]*margin-top\s*:\s*clamp\(4\.5rem,\s*8\.5vw,\s*7rem\))' 'assets/css/style.css equal half space below overview cards'
Assert-Matches $cssContent '(?is)\.home-overview__grid\s*\+\s*\.home-mode-sections::before\b(?=[^{}]*\{[^}]*display\s*:\s*block)(?=[^{}]*\{[^}]*height\s*:\s*clamp\(11\.25rem,\s*22\.5vw,\s*18rem\))(?=[^{}]*\{[^}]*margin-bottom\s*:\s*clamp\(4\.5rem,\s*8\.5vw,\s*7rem\))(?=[^{}]*\{[^}]*background\s*:\s*#f4f4f4)(?=[^{}]*\{[^}]*box-shadow\s*:\s*0\s+0\s+0\s+100vmax\s+#f4f4f4)' 'assets/css/style.css homepage separator plane'
